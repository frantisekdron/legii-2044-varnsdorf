...
const chapters = [
  ...
  {id: 'sklepy', name: 'Sklepy', title: 'Praktický prostor navíc.', text: 'Suterén prošel obnovou a získal novou betonovou podlahu. Třináct sklepních kójí a samostatný sklad doplňují kolárnu a prostornou dílnu.', before: '/media/reko-puvodni-sklep.webp', after: '/media/20-sklep-09-1.webp', beforeLabel: 'Původní stav', beforeAlt: 'Původní sklepní prostor před vybudováním kójí', afterAlt: 'Dokončená sklepní kóje s betonovou podlahou a obnoveným zdivem', position: '50% 50%', note: 'Proměna sklepního zázemí. Záběry dokumentují původní prostor a jednu z dnešních kójí, nikoli shodný kout.'},
];
const technical = [
...
function Comparison({chapter, onGallery}: {chapter: (typeof chapters)[number]; onGallery: (title: string, photos: Photo[]) => void}) {
  ...
  return <>
    ...
    <div className={'comparison-frame compare-' + mode}>
      {mode !== 'after' && <button className="comparison-image before-image" onClick={() => onGallery(chapter.name + ' — ' + chapter.beforeLabel.toLowerCase(), [{src: chapter.before}])} aria-label={'Zvětšit: ' + chapter.beforeAlt}><ResponsivePhoto sizes={mode === "both" ? "(max-width:600px) 86vw, 43vw" : "86vw"} src={chapter.before} alt={chapter.beforeAlt} style={{objectPosition: chapter.position}} loading="lazy"/><span className="comparison-label">{chapter.beforeLabel}</span><Maximize2 size={17}/></button>}
      {mode !== 'before' && <button className="comparison-image after-image" onClick={() => onGallery(chapter.name + ' — dnes', [{src: chapter.after}, ...(chapter.id === 'strecha' ? [{src: '/media/roof-panorama.webp'}, {src: '/media/roof-landscape.webp'}] : [])])} aria-label={'Zvětšit: ' + chapter.afterAlt}><ResponsivePhoto sizes={mode === "both" ? "(max-width:600px) 86vw, 43vw" : "86vw"} src={chapter.after} alt={chapter.afterAlt} loading="lazy"/><span className="comparison-label">Dnes</span><Maximize2 size={17}/></button>}
    </div>
...
export function Renovation({onGallery}: {onGallery: (title: string, photos: Photo[]) => void}) {
  return <section id="pribeh" className="section renovation-section" aria-labelledby="renovation-title">
    <div className="sketch-backdrop renovation-sketch" aria-hidden="true"><img src="/decor/reconstruction-tools.webp" alt="" width="1536" height="1024" loading="lazy" decoding="async"/></div>
