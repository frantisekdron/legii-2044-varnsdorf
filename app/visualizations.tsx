...
export function VisualizationGallery({items,onGallery}:{items:Visualization[];onGallery:(title:string,photos:{src:string;caption?:string}[])=>void}){
 ...
 {mode!=='stav'&&<button onClick={()=>gallery()} aria-label={'Zvětšit vizualizaci: '+item.title}><img src={item.thumb} srcSet={item.srcSet} sizes={mode==='srovnani'?'(max-width:760px) 86vw, 43vw':'86vw'} alt={item.title+' — návrh vybavení'} loading="lazy" decoding="async"/><span>{item.category==='podkrovi'?'Studie podkroví':'Vizualizace vybavení'}</span><Maximize2 size={18}/></button>}
 </div>{items.length>1&&<div className="viz-thumbs" aria-label="Výběr vizualizace">{items.map(p=><button key={p.id} aria-pressed={p.id===item.id} onClick={()=>setActive(p.id)}><img src={p.thumb} alt="" loading="lazy"/><span>{p.title}{items.some(other=>other.sceneId===p.sceneId&&other.id!==p.id)?` · ${p.variant}`:null}</span></button>)}</div>}</div>;
}
