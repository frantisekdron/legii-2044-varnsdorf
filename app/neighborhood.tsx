...
export function Neighborhood({onGallery}: {onGallery: (index: number) => void}) {
  return <section id="lokalita" className="neighborhood section" aria-labelledby="neighborhood-title">
    <div className="landscape-story">
      <div className="section-head location-heading"><div><p className="eyebrow">VARNSDORF · LEGIÍ 2044</p><h2 id="neighborhood-title">Na úpatí<br/><em>Lužických hor.</em></h2></div><div className="location-lead"><p>Městský život s kulturním a sportovním zázemím, Lužické hory a česko–německé příhraničí pro chvíle volna.</p><div className="location-actions"><button className="text-link" onClick={()=>onGallery(0)}>Prohlédnout okolí <ArrowRight size={17}/></button><a className="text-link" href="https://www.google.com/maps/search/?api=1&query=Legi%C3%AD+2044+Varnsdorf" target="_blank" rel="noreferrer">Zobrazit adresu na mapě <ArrowUpRight size={17}/></a></div></div></div>
