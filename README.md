...
## Approved visualizations in context

The redundant global inspiration section has been removed. Approved visualizations appear in matching unit details, the attic link and the existing amenity cards. Hero cards use selected living-room and staircase proposals when available. `app/visualizations-data.json` contains the approved publication set; the client never reads private review notes. Selected images support current-state / visualization / side-by-side views and appear in the matching unit's extra tab.

Read saved decisions from the separate review site, or use `vyber.json` inside its ZIP download. Run `scripts/import-selected-visualizations.py /absolute/path/vyber.json --dry-run` with the bundled Pillow Python first. Remove `--dry-run` to generate responsive WebP assets and replace the approved manifest, then use the usual Sites build and deployment workflow. The script rejects unsaved exports and revision mismatches, omits pending/rework decisions, and leaves scenes with notes for revision. It never publishes notes. Resolve notes before importing a new final selection. An empty import cannot silently clear the existing approved set; old image files remain available. Use `--revision N` only when the user explicitly approves importing a historical revision. Public titles do not expose A/B review labels.

Apartment types map 1→1/4/7, 2→2/5/8, 3→3/6/9. The bay-window bedroom of type 2 must never be assigned to unit 8. Attic scenes map to `puda`; staircase belongs to shared amenities. Original photo and staged image come from the same source scene, but attic revision 2 uses new study camera angles and is labeled as an unverified design concept. Approval on the review site is not an automatic deployment; the next authorized iteration imports the selected images and publishes the main site.
